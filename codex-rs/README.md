...
The same setting can be persisted in `~/.codexx/config.toml` (or `$CODEXX_HOME/config.toml`) via the top-level `sandbox_mode = "MODE"` key, e.g. `sandbox_mode = "workspace-write"`.
