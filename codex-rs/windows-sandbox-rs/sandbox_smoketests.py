...
def main() -> int:
    ...
    codex_home = Path(os.environ["USERPROFILE"]) / ".codexx"
    ...
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .codexx\\policy.json"], WS_ROOT)
    add("WS: .codexx cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .codexx policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
