...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".codexx")).await?;
    tokio::fs::create_dir_all(project_root.join(".codexx")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".codexx").join(CONFIG_TOML_FILE),
        "foo = \"root\"\n",
    )
    .await?;
    tokio::fs::write(
        nested.join(".codexx").join(CONFIG_TOML_FILE),
        "foo = \"child\"\n",
    )
    .await?;
    ...
    assert_eq!(
        ...
        nested.join(".codexx").as_path()
    );
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".codexx").as_path()
    );
...
"#;
    ...
    .await?;
    tokio::fs::write(nested.join(".codexx").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".codexx").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".codexx").join("child.txt"),
        "child instructions",
    )
    .await?;
